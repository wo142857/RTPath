...
function redis.reg_idle_cb(fcb, arg)
...
local function block_request(command, send, parse, client, ...)
...
-- nonblock request with idle callback 
local function nonblock_request(command, send, parse, client, ...)
	send(client, command, ...)
	client.network.socket:settimeout(0) --noblock

	while true do
		local reply = response.read(client)
		if type(reply) == "string" and reply == "timeout" then --EWOULDBLOCK
			idle_fcb(idle_arg)
		else
			if type(reply) == 'table' and reply.queued then
				reply.parser = parse
...
function response.read(client)
	...
	if idle_fcb ~= nil and type(payload) == "string" and payload == "timeout" then
		return "timeout"
	end
   ...
   -- bulk reply
    elseif prefix == '$' then
        ...
        if idle_fcb ~= nil then
            client.network.socket:settimeout(nil) --block
        end
...
local function custom(command, send, parse)
	...
	return function(client, ...)
			   if idle_fcb == nil then
				   return block_request(command, send, parse, client, ...)
			   else 
				   return nonblock_request(command, send, parse, client, ...)
			   end
		   end
end
...
function redis.connect(skt_class, ...)
