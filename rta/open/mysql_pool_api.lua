...
local APP_LINK_MYSQL_LIST = link["OWN_POOL"]["mysql"] or {}
...
local OWN_MYSQL_INDEX = {}


local function new_connect(sqlname)
	local nb = 0
	OWN_MYSQL_INDEX[sqlname] = (OWN_MYSQL_INDEX[sqlname] or 0)
	...
	repeat
		...
		if not ok then
			OWN_MYSQL_INDEX[sqlname] = OWN_MYSQL_INDEX[sqlname] + 1
			socket.select(nil, nil, 0.05 * ((OWN_MYSQL_INDEX[sqlname] >= MAX_DELAY) and MAX_DELAY or OWN_MYSQL_INDEX[sqlname]))
			only.log("E", OWN_MYSQL_POOLS[sqlname])
			only.log("I", string.format('MYSQL_NAME: %s | INDEX: %d |---> Tcp:connect: FAILED!', sqlname, OWN_MYSQL_INDEX[sqlname]))
	...
	only.log("I", string.format('MYSQL_NAME: %s | INDEX: %d |---> Tcp:connect: SUCCESS!', sqlname, OWN_MYSQL_INDEX[sqlname]))
	OWN_MYSQL_INDEX[sqlname] = 0
	return true
end
...
local function fetch_pool(sqlname)
	...
	if not OWN_MYSQL_POOLS[sqlname] then
		if not new_connect( sqlname ) then
			only.log("E", string.format("connect %s failed", sqlname))
			return false
		end
	end
	return true
