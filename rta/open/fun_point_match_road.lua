...
module("fun_point_match_road", package.seeall)
...
local function quick_location( lon, lat, dir, accountID, need )
	...
	local ok, old_roadID = redis_api.cmd("owner", accountID, "get", accountID .. ":sysInternalRoadID")
	...
	if need == 1 then
		...
		if new_roadID ~= old_roadID then
			redis_api.cmd("owner", accountID, "set", accountID .. ":sysInternalRoadID", new_roadID)
		end
	end
	return true,result
end
...
function detail_entry(direction, longitude, latitude, accountID, fields)
	...
	if (ok and result) and ((not fields) or (type(fields) == "table" and #fields > 0)) then
		if not fields then
			local ok, kv_tab = redis_api.cmd('mapRoadInfo', accountID or "", 'hgetall', result["roadID"] .. ":roadInfo")
			--only.log('D', string.format("hgetall %s counts is %d", key, utils.get_sha_tab_count(kv_tab)))
			if ok and kv_tab then
			...
			local ok, kv_tab = redis_api.cmd('mapRoadInfo', accountID or "", 'hmget', result["roadID"] .. ":roadInfo", fields)
			if ok and kv_tab and (#kv_tab > 0) then
				for i,val in ipairs(kv_tab) do
					result[ fields[i] ] = val
				end
